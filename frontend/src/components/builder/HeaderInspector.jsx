...
import { Plus, Trash2, ChevronDown, AlertTriangle, Image, Type, Layers, Ticket, User, ShoppingCart, CreditCard } from "lucide-react";
...
export const HeaderInspector = ({ device, sel, onSelect }) => {
  ...
  const rowClick = (el) => onSelect?.("header", el);
  const setRef = (el) => (node) => { if (node) rowRefs.current[el] = node; };

  return (
    <div className="space-y-2.5" data-testid="header-inspector">
      {/* ROW 1 — LOGO */}
      <RowCard testid="hdr-row-logo" active={sel?.element === "logo"} rowRef={setRef("logo")} onClick={() => rowClick("logo")} dragHandleProps={{}}>
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1.5 text-sm font-semibold text-[#0B1510]"><Image size={14} className="text-[#3FC16F]" /> Logo</span>
          <span className="text-[10px] font-semibold uppercase text-[#B7C2BA]">1º · Izquierda</span>
        </div>
        {sel?.element === "logo" && (
          <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            <Segmented value={h.logo.position} onChange={(v) => setLogo({ position: v })} testid="logo-position"
              options={[{ value: "inside", label: "Dentro" }, { value: "outside", label: "Sobresale" }]} />
      ...
      <div className="flex items-center justify-between px-1 pt-1">
        ...
        <div className="w-28"><FitMeter status={status} count={navCount} max={NAV_MAX} /></div>
      </div>
      ...
      {navCount >= 6 && (
        <div className={cx("flex items-start gap-2 rounded-lg px-3 py-2 text-[11px] font-medium", navCount >= NAV_MAX ? "bg-[#FDECEC] text-[#C4362F]" : "bg-[#FFF6E5] text-[#8A6300]")} data-testid="nav-capacity-warning">
          ...
          {navCount >= NAV_MAX ? "Límite alcanzado" : "No caben más enlaces en desktop — quita uno o acorta el texto"}
        </div>
      )}
      {navCount > 5 && navCount < NAV_MAX && (
        <p className="px-1 text-[10px] text-[#6B7A70]">Fuente reducida a {fontNow}px para que quepan.</p>
      )}

      {h.navLinks.map((l, i) => (
        <RowCard key={i} testid={`hdr-row-nav-${i}`} active={sel?.element === `nav-${i}`} rowRef={setRef(`nav-${i}`)} onClick={() => rowClick(`nav-${i}`)} dragHandleProps={navDnd.handleProps(i)}>
      ...
      <button onClick={addLink} disabled={navCount >= NAV_MAX} data-testid="nav-add"
        className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-[#CBD6CF] py-2.5 text-xs font-semibold text-[#4B5A50] transition-colors hover:bg-[#F1F5F2] disabled:opacity-40">
      ...
      <div className="flex items-center justify-between px-1 pt-2">
        ...
        <span className="text-[11px] font-semibold text-[#6B7A70]" data-testid="action-count">{actCount}/{aMax}</span>
      </div>
      {actCount > aMax && (
        <div className="flex items-start gap-2 rounded-lg bg-[#FFF6E5] px-3 py-2 text-[11px] font-medium text-[#8A6300]" data-testid="action-capacity-warning">
          <AlertTriangle size={13} className="mt-0.5" /> {device === "mobile" ? "Máx 2 acciones en móvil" : "Máx 3 acciones en desktop"}
        </div>
      )}
      ...
      {h.actions.map((a, i) => {
        ...
        return (
          <RowCard key={i} testid={`hdr-row-action-${i}`} active={sel?.element === `action-${i}`} rowRef={setRef(`action-${i}`)} onClick={() => rowClick(`action-${i}`)} dragHandleProps={actDnd.handleProps(i)}>
            <div {...actDnd.rowProps(i)}>
              ...
              {device === "mobile" && (
                <span className="mt-1.5 inline-block rounded-full bg-[#EAF2EC] px-2 py-0.5 text-[10px] font-semibold text-[#4B5A50]" data-testid={`action-mobile-badge-${i}`}>Móvil: iconos</span>
              )}
              ...
              {sel?.element === `action-${i}` && (
                <div className="mt-2.5 space-y-2.5" onClick={(e) => e.stopPropagation()}>
                  <Segmented value={a.type} onChange={(v) => updateAction(i, { type: v })} testid={`action-type-${i}`} cols={3}
                    options={[{ value: "button", label: "Botón" }, { value: "icon", label: "Icono" }, { value: "icon-badge", label: "Badge" }]} />
      ...
      <div className="flex flex-wrap gap-1.5" data-testid="action-presets">
        {ACTION_PRESETS.map((p) => (
          <button key={p.key} onClick={() => addPreset(p)} disabled={actCount >= 3} data-testid={`action-preset-${p.key}`}
            className="inline-flex items-center gap-1.5 rounded-full border border-[#E3E9E5] px-3 py-1.5 text-xs font-semibold text-[#4B5A50] transition-colors hover:border-[#3FC16F] hover:bg-[#EAF7EF] disabled:opacity-40">
            <p.Icon size={13} /> {p.label}
          </button>
        ))}
      </div>

      {/* HEADER SHELL — collapsed accordion */}
