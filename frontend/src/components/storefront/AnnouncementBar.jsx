...
export const AnnouncementBar = ({ config, onDismiss, dismissed, editable, selected, onSelect }) => {
    ...
    >
          ...
          <div className="relative flex min-h-[18px] w-full items-center justify-center">
            {list.map((a, i) => (
              <div key={i} className="absolute inset-0 flex items-center justify-center transition-all duration-500"
                style={transition === "fade" ? { opacity: i === index ? 1 : 0 } : { transform: `translateY(${(i - index) * 100}%)`, opacity: i === index ? 1 : 0 }}>
                {renderItem(a, i)}
              </div>
            ))}
      ...
      {list.length > 1 && transition !== "marquee" && (
        <div className="absolute left-4 top-1/2 hidden -translate-y-1/2 items-center gap-1 sm:flex">
          {list.map((_, i) => (
            <button key={i} onClick={(e) => { e.stopPropagation(); setIndex(i); }} className="h-1.5 w-1.5 rounded-full transition-opacity" style={{ backgroundColor: config.textColor, opacity: i === index ? 1 : 0.4 }} aria-label={`Anuncio ${i + 1}`} />
          ))}
        </div>
      )}

      {config.dismissible && (
        <button onClick={(e) => { e.stopPropagation(); onDismiss?.(); }} data-testid="store-announcement-dismiss" className="absolute right-3 top-1/2 -translate-y-1/2 opacity-70 hover:opacity-100 transition-opacity" style={{ color: config.textColor }} aria-label="Cerrar">
          <X size={15} />
        </button>
      )}
    </div>
